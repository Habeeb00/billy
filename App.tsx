...
function Snowflake({ style }: { style: React.CSSProperties }) {
    return <div className="absolute text-white text-2xl select-none" style={style}>❄</div>;
}
...
function DynamicBackgroundComponent({ theme, animationsEnabled }: { theme: Theme, animationsEnabled: boolean }) {
    ...
    const birds = Array.from({ length: 3 }).map(() => ({
    ...
    const snowflakes = Array.from({ length: 70 }).map(() => ({
        left: `${Math.random() * 100}vw`,
        top: '-10vh',
...
const DynamicBackground = DynamicBackgroundComponent;
...
function App() {
  ...
  return (
    <main className={`relative w-screen h-screen overflow-hidden`}>
      {/* Background Layer: Contains background color and all animated elements. */}
      <div className={`absolute inset-0 transition-colors duration-1000 ${BG_COLORS[theme]}`}>
        <DynamicBackground theme={theme} animationsEnabled={animationsEnabled} />
      </div>

      {/* UI Layer: Renders on top of the background. */}
        ...
        <div className="relative flex flex-col items-center">
          ...
          <div className="bg-[#4a4a4a] p-2 sm:p-3 border-4 border-black shadow-[8px_8px_0px_rgba(0,0,0,0.7)]">
           ...
           {selectedPlots.length > 0 && (
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 z-10 flex items-center gap-2">
                <button
